...
from genesis.sensors import (RecordingOptions, RigidContactForceGridSensor,
                             SensorDataRecorder)
    ...
    for x in range(grid_data.shape[1]):
        for y in range(grid_data.shape[2]):
            for z in range(grid_data.shape[3]):
                ...
                # Skip cells with no force for better visualization
...
def main():
    ...
    parser.add_argument("--debug", "-d", action="store_true", help="Draw the contact grid sensor debug objects")
    ...
    data_recorder.add_sensor(
        cam,
        RecordingOptions(
            handler=VideoFileWriter(filename="grid_test.mp4", fps=1 / args.dt)
        ),
    )
    data_recorder.add_sensor(

...
import numpy as np
import trimesh
from genesis.sensors import RecordingOptions, RigidNormalTangentialForceGridSensor, SensorDataRecorder
...
def visualize_grid_sensor(scene: gs.Scene, sensor: RigidNormalTangentialForceGridSensor, min_force=0.0, max_force=1.0):
    """
    ...
    for x in range(grid_data.shape[1]):
        for y in range(grid_data.shape[2]):
            for z in range(grid_data.shape[3]):
                norm_tan = grid_data[0, x, y, z]
                force_magnitude = norm_tan[0]
...
def main():
    ...
    # biig box
    scene.add_entity(
        morph=gs.morphs.Box(pos=(0.0, 0.0, 1.0), size=(1.00, 1.00, 0.1)),
    )
    ...
    for i in range(args.b):
        ...
        blocks.append(block)
        sensors.append(RigidNormalTangentialForceGridSensor(entity=block, grid_size=(1, 1, 2)))

...
def test_scene(args, sensor_type):
    ...
    print(f"Average time per step: {total_time / steps:.4f} seconds")
...
if __name__ == "__main__":
    ...
    new_t = test_scene(args, sensor_type=RigidContactForceGridSensor)
    print()
    print("Testing old RigidContactForceGridSensor...")
    old_t = test_scene(args, sensor_type=OldRigidContactForceGridSensor)

    print("batch size:", args.n_envs, "blocks:", args.b, "grid size:", args.grid_size)

    # print in table format average time per step for both sensors
    print(f"{'Sensor Type':<40} {'Total Time (s)':<20} {'Avg Time per Step (s)':<20}")
    print(f"{'Old RigidContactForceGridSensor':<40} {old_t:<20.4f} {old_t / (args.seconds / args.dt):<20.4f}")
    print(f"{'New RigidContactForceGridSensor':<40} {new_t:<20.4f} {new_t / (args.seconds / args.dt):<20.4f}")

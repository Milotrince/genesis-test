...
def visualize_grid_sensor(
    scene: gs.Scene, sensor: RigidContactForceGridSensor, min_force=0.0, max_force=1.0
):
    """
    ...
    link_pos = tensor_to_array(
        scene._sim.rigid_solver.get_links_pos(links_idx=sensor.link_idx).squeeze(axis=1)
    )
    link_quat = tensor_to_array(
        scene._sim.rigid_solver.get_links_quat(links_idx=sensor.link_idx).squeeze(
            axis=1
        )
    )
    ...
    for x in range(grid_data.shape[1]):
        for y in range(grid_data.shape[2]):
            for z in range(grid_data.shape[3]):
                ...
                color_intensity = (
                    np.clip(force_magnitude, min_force, max_force) / max_force
                )
                ...
                local_pos = (
                    sensor.min_bounds + (np.array([x, y, z]) + 0.5) * grid_cell_size
                )
...
def main():
    ...
    parser.add_argument(
        ...
        "-d",
        action="store_true",
        help="Draw the contact grid sensor debug objects",
    )
    ...
    data_recorder.add_sensor(
        cam,
        RecordingOptions(
            handler=VideoFileWriter(filename="hand_test.mp4", fps=30)
        ),
    )
    sensors = []
    for link in hand.links:
        if link.name in sensorized_link_names:
            sensor = RigidContactForceGridSensor(
                entity=hand, link_idx=link.idx, grid_size=(2, 2, 2)
            )
            data_recorder.add_sensor(
                sensor,
                RecordingOptions(
                    handler=NPZFileWriter(filename=f"{link.name}_contact_forces.npz")
                ),
            )

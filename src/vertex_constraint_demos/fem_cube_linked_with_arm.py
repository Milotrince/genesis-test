...
def main():
    ...
    try:
        ...
        for i in range(100):
            arm.set_dofs_position(np.array(
                [0.9643, -0.3213, -0.6685, -2.3139, -0.2890,  2.0335, -1.6014,  0.0306, 0.0306]
            ), dofs_idx_local)
            scene.step()
        ...
        qpos = arm.inverse_kinematics(
            link=end_joint.link,
            pos=np.array(arm_target_pos),
            quat=np.array([0, 1, 0, 0]),
        )
        arm_path_waypoints = arm.plan_path(
            qpos_goal=qpos,
            num_waypoints=int(1 / dt)
        )
        ...
        cube.solver.remove_all_vertex_constraints()
        steps = int(1 / dt)
        ...
        video_filename = f"out/arm_link_{args.solver}_dt={dt}_substeps={substeps}.mp4"
